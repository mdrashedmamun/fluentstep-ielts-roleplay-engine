...
/**
 * Phase 8 Step 3: Generate Blank-Inserted Unit 4 Scenarios
...
const DIALOGUE_1_ANSWERS = {
  ...
  3: { phrase: 'point', bucket: 'BUCKET_A', score: 50, alternatives: ['observation', 'perspective', 'view'] },
  4: { phrase: 'encounter', bucket: 'BUCKET_A', score: 45, alternatives: ['experience', 'have', 'see'] },
  5: { phrase: 'rapport', bucket: 'BUCKET_B', score: 30, alternatives: ['connection', 'relationship', 'bond'] },
  6: { phrase: 'diminished', bucket: 'BUCKET_A', score: 45, alternatives: ['reduced', 'weakened', 'lessened'] },
  7: { phrase: 'intentionally', bucket: 'BUCKET_B', score: 28, alternatives: ['deliberately', 'on purpose', 'purposefully'] },
  8: { phrase: 'valid', bucket: 'BUCKET_A', score: 48, alternatives: ['good', 'sound', 'reasonable'] }
};
...
const DIALOGUE_2_ANSWERS = {
  ...
  3: { phrase: 'created', bucket: 'BUCKET_A', score: 46, alternatives: ['generated', 'produced', 'made'] },
  4: { phrase: 'unprecedented', bucket: 'BUCKET_B', score: 32, alternatives: ['unparalleled', 'extraordinary', 'remarkable'] },
  5: { phrase: 'adapt', bucket: 'BUCKET_A', score: 50, alternatives: ['adjust', 'acclimate', 'evolve'] },
  6: { phrase: 'positive momentum', bucket: 'BUCKET_A', score: 48, alternatives: ['progress', 'advancement', 'improvement'] },
  7: { phrase: 'acknowledge', bucket: 'BUCKET_A', score: 46, alternatives: ['recognize', 'admit', 'concede'] },
  8: { phrase: 'opportunity', bucket: 'BUCKET_A', score: 50, alternatives: ['chance', 'possibility', 'prospect'] }
};
...
const DIALOGUE_3_ANSWERS = {
  ...
  3: { phrase: 'aligned', bucket: 'BUCKET_A', score: 46, alternatives: ['connected', 'related', 'associated'] },
  4: { phrase: 'mixed', bucket: 'BUCKET_B', score: 28, alternatives: ['inconclusive', 'unclear', 'ambiguous'] },
  5: { phrase: 'constraints', bucket: 'BUCKET_B', score: 32, alternatives: ['limitations', 'restrictions', 'obstacles'] },
  6: { phrase: 'devastating', bucket: 'BUCKET_A', score: 48, alternatives: ['catastrophic', 'terrible', 'ruinous'] },
  7: { phrase: 'assurance', bucket: 'BUCKET_A', score: 46, alternatives: ['guarantee', 'proof', 'evidence'] },
  8: { phrase: 'comprehensively', bucket: 'BUCKET_B', score: 30, alternatives: ['thoroughly', 'carefully', 'systematically'] }
};
...
const DIALOGUE_4_ANSWERS = {
  ...
  3: { phrase: 'incompetent', bucket: 'BUCKET_B', score: 32, alternatives: ['incapable', 'unable', 'unqualified'] },
  4: { phrase: 'extensive', bucket: 'BUCKET_B', score: 30, alternatives: ['significant', 'substantial', 'considerable'] },
  5: { phrase: 'exposed', bucket: 'BUCKET_A', score: 44, alternatives: ['subjected', 'vulnerable', 'open'] },
  6: { phrase: 'process', bucket: 'BUCKET_A', score: 50, alternatives: ['understand', 'grasp', 'comprehend'] },
  7: { phrase: 'observation', bucket: 'BUCKET_A', score: 46, alternatives: ['point', 'comment', 'note'] },
...
function generateUnit4Scenarios() {
  ...
  return dialogueData.map((data, idx) => {
    ...
    // Calculate metrics
    const bucketACount = answerVariations.filter(a => a.bucket === 'BUCKET_A').length;
    const bucketBCount = answerVariations.filter(a => a.bucket === 'BUCKET_B').length;
    const novelCount = answerVariations.filter(a => a.bucket === 'NOVEL').length;
    ...
    const deepDive = answerVariations.map(av => ({
      index: av.index,
      phrase: av.answer,
      insight: DEEP_DIVE_INSIGHTS[av.answer] || `C1-C2 level vocabulary for advanced learners. Context-dependent usage.`
    }));
...
function generateBlankInsertionReport(scenarios) {
  ...
  scenarios.forEach((scenario, idx) => {
    ...
    report += `- **BUCKET_A**: ${scenario.metrics.bucketA} (${Math.round((scenario.metrics.bucketA / scenario.metrics.totalBlanks) * 100)}%)\n`;
    report += `- **BUCKET_B**: ${scenario.metrics.bucketB} (${Math.round((scenario.metrics.bucketB / scenario.metrics.totalBlanks) * 100)}%)\n`;
    report += `- **Novel**: ${scenario.metrics.novel} (${Math.round((scenario.metrics.novel / scenario.metrics.totalBlanks) * 100)}%)\n`;
    ...
    scenario.answerVariations.slice(0, 3).forEach(av => {
      report += `- **${av.index}. ${av.answer}** [${av.bucket}] → ${av.alternatives.slice(0, 2).join(', ')}\n`;
    });
    report += `\n`;
  });

  const totalBlanks = scenarios.reduce((sum, s) => sum + s.metrics.totalBlanks, 0);
  const totalBucketA = scenarios.reduce((sum, s) => sum + s.metrics.bucketA, 0);
  const avgCompliance = Math.round(scenarios.reduce((sum, s) => sum + s.metrics.complianceScore, 0) / scenarios.length);
  ...
  report += `| **BUCKET_A %** | ${Math.round((totalBucketA / totalBlanks) * 100)}% | 65-75% |\n`;
  ...
  report += `✅ BUCKET_A compliance: ${Math.round((totalBucketA / totalBlanks) * 100)}% (target: 65-75%)\n`;
