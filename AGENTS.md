# FluentStep - Codex Configuration

**Project**: IELTS Roleplay Engine with Multi-Agent Content Generation
**Status**: Private-beta QA candidate in progress (53 scenarios; not production-approved)
**Agents**: 11 active (5 archived to context reduction)
**Last Updated**: Jun 3, 2026 (Long-horizon QA claim-boundary refresh)
...
|------|---------|------------|
| `AGENTS.md` | This file - navigation index | ✅ Yes |
| `Codex.local.md` | Personal session notes | ❌ No (.gitignore) |
...
Formalized, discoverable skills in `.Codex/skills/`:
...
| Metric | Value | Status |
|--------|-------|--------|
| Total Scenarios | 53 | Local validators passing; human approval still required |
| Test Pass Rate | Tier 1 local E2E 71/71 in latest focused run | Private-beta verifier evidence only |
| Build Status | Build/type-check passing locally | Not deploy approval |
...
For personal session notes, use `.Codex/Codex.local.md` (gitignored):
```bash
# Edit personal notes (not committed)
nano .Codex/Codex.local.md
```
...
2. **File structure**: All `.Codex/rules/` files present
3. **Skills registered**: All `.Codex/skills/` files present
4. **Git status**: `.Codex/Codex.local.md` untracked (gitignored)
...
rm -rf .Codex/rules/ .Codex/skills/
...
**Maintained by**: Codex Architecture Team
