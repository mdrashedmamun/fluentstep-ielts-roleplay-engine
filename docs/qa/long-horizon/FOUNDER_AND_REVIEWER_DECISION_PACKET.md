...
- Preview deployment: Ready at `https://fluentstep-ielts-roleplay-engine-2pgzvm9wy.vercel.app`; `curl -I` returned HTTP 200. Remote `qa:browser` is not green because Node fetch cannot reach the preview in this local environment, so preview is HTTP-checked but not browser-smoke-approved.
