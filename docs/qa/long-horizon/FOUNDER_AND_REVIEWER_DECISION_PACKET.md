...
- Browser QA: passing locally; latest run `2026-06-03T13:52:17Z`; 12 screenshots including Healthcare disclaimer evidence and blank-integrity regression evidence, 0 automated issues, 0 console errors, and 0 failed responses.
...
- Visual layout lint: current run `2026-06-03T13:52:46Z` exits 0 through screenshot fallback because Chromium DOM lint cannot register the macOS MachPort under sandbox permissions. Treat as automated evidence, not visual approval.
- Preview deployment: branch pushes create Vercel previews; verify the current URL with `npx vercel ls fluentstep-ielts-roleplay-engine` and `npx vercel inspect <url>`. The latest inspected preview during this pass reached Ready and `curl -I` returned HTTP 200. Remote `qa:browser` is not green because Node fetch cannot reach the preview in this local environment, so preview is HTTP-checked but not browser-smoke-approved.
- Tier 1 local E2E: passing locally after the blank-integrity test expectation update. Focused multiple-blank rerun passed 2/2 selected cases, and full Tier 1 rerun passed 71/71 with 3 inherited pytest warnings in 234.23s.
