...
export const Card: React.FC<CardProps> = ({
  ...
  const variantClasses: Record<CardVariant, string> = {
    story: 'h-96 rounded-2xl shadow-lg hover:shadow-xl hover:shadow-indigo-100 transition-all duration-300 border border-slate-100 flex flex-col p-8 hover:-translate-y-1 cursor-pointer',
  ...
  const interactiveClasses = interactive && variant !== 'story'
    ? 'hover:border-indigo-300 hover:shadow-md cursor-pointer transition-all'
    : '';
